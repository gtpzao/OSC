...
This repository is an OSC-specific Paperclip fork focused on a lower-friction default stack:

- bundled `Software Factory Method` BMAD company starter
- `claude_local` wired to `fcc-claude` by default
- NVIDIA NIM as the default free-entry model path
- ORK visual layer on top of the Paperclip UI and flows

First-run OSC flow:

1. Start OSC in local mode.
2. On the first empty local boot, OSC auto-bootstraps the bundled Software Factory Method with the default FCC + NVIDIA NIM runtime.
3. Open the board and verify the factory agents are already available.
4. Use ORK onboarding when you want to customize the workspace/runtime, create another factory, or open the generated `BMAD Launchpad` issue.

Use `Test now` before saving onboarding changes. If the probe says `fcc-claude` is missing, onboarding shows the Windows FCC installer command and also accepts an absolute path to the installed launcher.

Fork-specific guides:

- [OSC local runtime](docs/guides/osc-local-runtime.md)
- [OSC safety and autonomous mode](docs/guides/osc-safety.md)

The intent is to preserve the Paperclip control-plane model while removing the upfront token/subscription barrier for a first local install.
