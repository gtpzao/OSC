...
const orkStarterCardClasses =
  "overflow-hidden rounded-none border border-primary/20 bg-[linear-gradient(180deg,rgba(24,36,16,0.94),rgba(11,17,8,0.96))] shadow-[0_24px_64px_rgba(0,0,0,0.34),inset_0_1px_0_rgba(196,255,103,0.06)]";
...
function OnboardingRoutePage() {
  ...
  const title = matchedCompany
    ...
    : companies.length > 0
      ? "Create another ORK factory"
      : "Launch your ORK factory";
  const description = matchedCompany
    ? "Run onboarding again to add an agent and a starter task for this company."
    : companies.length > 0
      ? "Run onboarding again to spin up another factory with the bundled BIRCK baseline."
      : "Install the bundled BIRCK baseline and wire the default FCC + NVIDIA NIM runtime.";

  return (
    <div className="mx-auto max-w-xl py-10">
      <div className={orkStarterCardClasses}>
        <div className="border-b border-primary/10 px-6 py-5">
          <h1 className="text-base font-semibold uppercase tracking-[0.16em] text-primary">{title}</h1>
        </div>
        <div className="px-6 py-5">
          ...
          <div className="mt-4">
            ...
            >
              {matchedCompany ? "Add Agent" : "Launch ORK Onboarding"}
            </Button>
          </div>
        </div>
      </div>
...
function NoCompaniesStartPage() {
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className={orkStarterCardClasses}>
        <div className="border-b border-primary/10 px-6 py-5">
          <h1 className="text-base font-semibold uppercase tracking-[0.16em] text-primary">
            {t("app.noCompanies.title", { defaultValue: "Launch your ORK factory" })}
          </h1>
        </div>
        <div className="px-6 py-5">
          <p className="mt-2 text-sm text-muted-foreground">
            {t("app.noCompanies.description", { defaultValue: "Install the bundled BIRCK baseline with the free FCC + NVIDIA NIM default stack." })}
          </p>
          <div className="mt-4">
            <Button onClick={() => openOnboarding()}>
              {t("app.noCompanies.newCompany", { defaultValue: "Launch ORK Onboarding" })}
            </Button>
          </div>
        </div>
      </div>
