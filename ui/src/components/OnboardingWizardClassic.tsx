 ...
 * (PAP-136 / PAP-138, plan §3 Tier B).
 *
 * Source: `git show master:ui/src/components/OnboardingWizard.tsx` at commit
 * 2e74d32871b3fbb1c57cfe5d373b323c8e6ee93a ("PAP-10430: split Issue-to-Task
 * copy migration (#7651)"). Only the component name was changed; everything
 * else is byte-identical so the flag-off experience stays pixel-identical to
 * master. Do NOT evolve this copy — it is rendered only while the
 * "Conference Room Chat" experimental flag is off and gets deleted when the
...
export function OnboardingWizardClassic() {
  ...
  const [command, setCommand] = useState(DEFAULT_FCC_COMMAND);
    ...
    try {
      ...
      if (isLocalAdapter) {
        ...
        if (result.status !== "pass") {
          setError("Adapter environment check failed. Fix the runtime before creating the first agent.");
          return;
        }
      }
