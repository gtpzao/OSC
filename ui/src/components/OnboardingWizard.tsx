...
import {
  FCC_WINDOWS_INSTALL_COMMAND,
  isFccClaudeCommand,
} from "../lib/fcc-runtime";
...
const OSC_FACTORY_STARTER_ID = "birck";
const DEFAULT_ORK_LEAD_NAME = "ORK Lead";
const DEFAULT_FCC_COMMAND = "fcc-claude";
const DEFAULT_NVIDIA_NIM_MODEL =
...
function buildBmadLaunchpadDescription(input: {
  companyName: string;
  companyGoal: string;
  agentName: string;
}) {
  return [
    `Operate as the BMAD factory lead for ${input.companyName}. Turn the company mission into an executable delivery plan.`,
    "",
    `Mission: ${input.companyGoal}`,
    "",
    "Required outcome:",
    "1. Review the active company goal and the installed BIRCK starter setup.",
    "2. Produce or update the core planning artifacts needed to execute the mission.",
    "3. Break the work into the next epics, stories, or issues and delegate them to the right factory agents.",
    "4. Call out blockers, assumptions, missing repo context, and runtime risks.",
    `5. Leave a clear summary for the board after ${input.agentName} routes the first wave of work.`,
    "",
    "Use the installed BMAD and Paperclip skills instead of improvising when they apply.",
  ].join("\n").trim();
}
...
export function OnboardingWizard() {
  ...
  const [agentName, setAgentName] = useState(
    (saved?.agentName as string) || DEFAULT_ORK_LEAD_NAME
  );
  ...
  const [model, setModel] = useState(
    (saved?.model as string) || DEFAULT_NVIDIA_NIM_MODEL
  );
  const [command, setCommand] = useState(
    (saved?.command as string) || DEFAULT_FCC_COMMAND
  );
  ...
  useEffect(() => {
    ...
    const state = {
      ...
      createdCompanyId, createdCompanyPrefix, createdCompanyGoalId, createdAgentId, createdProjectId, createdIssueRef,
  ...
  const COMMAND_PLACEHOLDERS: Record<string, string> = {
    claude_local: DEFAULT_FCC_COMMAND,
    codex_local: "codex",
    gemini_local: "gemini",
    hermes_local: "hermes",
    pi_local: "pi",
  ...
  const showFccLauncherGuidance =
    adapterType === "claude_local" &&
    isFccClaudeCommand(effectiveAdapterCommand);
  ...
  function reset() {
    ...
    setQ3("");
    setQ4("");
    setAgentName(DEFAULT_ORK_LEAD_NAME);
    setAdapterType("claude_local");
    setModel(DEFAULT_NVIDIA_NIM_MODEL);
    setCommand(DEFAULT_FCC_COMMAND);
    setArgs("");
    setUrl("");
    setAdapterEnvResult(null);
    setAdapterEnvError(null);
  ...
  async function handleLaunchToIssue() {
    ...
    try {
      ...
      if (!issueRef) {
        const issue = await issuesApi.create(
          createdCompanyId,
          buildOnboardingIssuePayload({
            title: `Run BMAD intake for ${companyName.trim()}`,
            description: buildBmadLaunchpadDescription({
              companyName: companyName.trim(),
              companyGoal: companyGoal.trim(),
      ...
      setError(err instanceof Error ? err.message : "Failed to create BMAD launchpad issue");
    } finally {
      setLoading(false);
    }
  }
  ...
  // Step 4 → 5: install the bundled factory starter into the new company.
    ...
    try {
      ...
      if (isLocalAdapter) {
        ...
        if (result.status !== "pass") {
          setError("Adapter environment check failed. Fix the runtime before installing the factory.");
          return;
        }
      }

      const imported = await companiesApi.installStarter(createdCompanyId, {
        starterId: OSC_FACTORY_STARTER_ID,
        adapterType,
      ...
      const leadAgent =
        imported.agents.find((agent) => agent.slug === "ceo" && agent.id) ??
        imported.agents.find((agent) => agent.id);
      if (!leadAgent?.id) {
        throw new Error(
          "Factory starter installed, but no lead agent was returned."
        );
      }
      setCreatedAgentId(leadAgent.id);
      queryClient.invalidateQueries({
      ...
      setError(
        err instanceof Error ? err.message : "Failed to install factory starter"
      );
    } finally {
      setLoading(false);
    }
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) {
      ...
      else if (step === 5) handleLaunchToIssue();
    }
  }
        ...
        <div className="fixed inset-0 z-50 flex" onKeyDown={handleKeyDown}>
          ...
          >
            <div className="w-full max-w-md mx-auto my-auto px-8 py-12 shrink-0">
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">
                        {step === 3
                          ? "Name your factory lead"
                          : step === 4
                            ? "Wire the factory runtime"
                            : "Review"}
                      </h3>
                      <p className="text-xs text-muted-foreground">
                        {step === 3 ? (
                          <>
                            Name the CEO imported from the{" "}
                            <span className="font-medium text-foreground">
                              BIRCK
                            </span>{" "}
                            starter. It will drive{" "}
                            <span className="font-medium text-foreground">{companyName}</span>{" "}
                            toward its mission. We default to{" "}
                            <span className="font-medium text-foreground">
                              {DEFAULT_ORK_LEAD_NAME}
                            </span>
                            .
                          </>
                        ) : step === 4 ? (
                          <>
                            Set the runtime once. OSC will apply it to the whole
                            imported BMAD company.
                          </>
                        ) : (
                          <>
                            The starter company is installed and your factory
                            lead is ready to run.
                          </>
                        )}
                      </p>
                    </div>
                  ...
                  <div className="flex flex-col items-center gap-1.5 py-1 text-center">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {step === 3 ? (
                        "the slot reserved for your imported factory lead"
                      ) : step === 4 ? (
                        "the Software Factory runtime, taking shape"
                      ) : (
                        <>
                          <span className="font-medium text-foreground">{agentName}</span>{" "}
                          is online with the rest of your BMAD factory.
                        </>
                      )}
                    </p>
                  </div>
              ...
              {step === 2 && onboardingPath === "grow" && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                      <h3 className="font-medium">Tell us about your factory</h3>
                      <p className="text-xs text-muted-foreground">
                        We'll use this to set up your factory lead and plan which agents to add.
                      </p>
                    </div>
                  </div>
                  <div className="group">
                    <label className="text-xs text-muted-foreground mb-1 block">What does your factory work on?</label>
                    <input
              ...
              {/* Step 1: Name your factory (both paths) */}
              {step === 1 && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 mb-1">
                    <div className="bg-muted/50 p-2">
                    ...
                    <div>
                      <h3 className="font-medium">Name your ORK factory</h3>
                      <p className="text-xs text-muted-foreground">
                        What should we call this factory?
                      </p>
                    </div>
                  </div>
                  ...
                  {missionPath === "direct" && (
                    <div className="space-y-3 animate-in fade-in duration-200">
                      <div className="group">
                        ...
                        <textarea
                          className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50 resize-none min-h-[60px]"
                          placeholder="What is this factory trying to achieve?"
                          value={companyGoal}
                          onChange={(e) => setCompanyGoal(e.target.value)}
                          autoFocus
                        />
                    ...
                    <div className="space-y-3 animate-in fade-in duration-200">
                      <div className="group">
                        <label className="text-xs text-muted-foreground mb-1 block">
                          What does your factory work on?
                        </label>
                        <input
                          className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                          placeholder="e.g. We create educational YouTube content about AI"
              ...
              {/* Step 3: Name the factory lead — name only (capsule above) */}
              {step === 3 && (
                <div className="space-y-5">
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">
                    ...
                    <input
                      className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                      placeholder={DEFAULT_ORK_LEAD_NAME}
                      value={agentName}
              ...
              {/* Step 4: Connect the factory runtime — adapter + model + env check */}
              {step === 4 && (
                <div className="space-y-5">
                      ...
                      {recommendedAdapters.map((opt) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setCommand(
                              COMMAND_PLACEHOLDERS[nextType] ??
                                nextType.replace(/_local$/, "")
                            );
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={COMMAND_PLACEHOLDERS[adapterType] ?? adapterType.replace(/_local$/, "")}
                          value={command}
                          onChange={(e) => setCommand(e.target.value)}
                        />
                        <p className="mt-1 text-[11px] text-muted-foreground">
                          Use an absolute path if the adapter CLI is not available on your PATH.
                        </p>
                      </div>
                      <div>
                        <label className="text-xs text-muted-foreground mb-1 block">
                        ...
                        <input
                          className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm font-mono outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                          placeholder="e.g. --debug, --some-flag=value"
                          value={args}
                          onChange={(e) => setArgs(e.target.value)}
                        />
                        <p className="mt-1 text-[11px] text-muted-foreground">
                          Appends flags after Paperclip&apos;s managed CLI args.
                        </p>
                      </div>
                    </div>
                  )}

                  {showFccLauncherGuidance && (
                    <div className="rounded-md border border-border/70 bg-muted/20 px-3 py-2 space-y-1.5">
                      <p className="text-xs font-medium">FCC default runtime</p>
                      <p className="text-[11px] text-muted-foreground leading-relaxed">
                        ORK expects Free Claude Code installed and <span className="font-mono">fcc-server</span> running.
                        Install or update the Windows launcher with:
                      </p>
                      ...
                      {adapterEnvResult && adapterEnvResult.status === "fail" && (
                        <div className="rounded-md border border-border/70 bg-muted/20 px-2.5 py-2 text-[11px] space-y-1.5">
                          ...
                          adapterType === "opencode_local" ? (
                            ...
                            <p className="text-muted-foreground">
                              {effectiveAdapterCommand === DEFAULT_FCC_COMMAND ? (
                                <>
                                  Keep <span className="font-mono">fcc-server</span>{" "}
                                  running and confirm the FCC Admin UI has a
                                  valid NVIDIA NIM key and model before retrying.
                                </>
                              ) : (
                                <>
              ...
              {step === 5 && (
                <div className="space-y-5 py-1">
                  ...
                  <div className="space-y-1.5">
                    {[
                      { label: "Company named", done: Boolean(companyName.trim()) },
                      { label: "Mission", done: Boolean(companyGoal.trim()) },
                      { label: "BIRCK starter installed", done: Boolean(createdAgentId) },
                      { label: "FCC + model runtime connected", done: Boolean(createdAgentId) },
                    ].map(({ label, done }) => (
                  ...
                  <p className="text-xs text-muted-foreground text-center">
                    Open a BMAD launchpad issue to kick off planning and delegation,
                    or jump into board chat to direct the factory manually.
                  </p>
                </div>
              )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {step === 4 && (
                    ...
                    >
                      ...
                      {loading ? "Installing..." : "Install the factory"}
                    </Button>
                  )}
                  {step === 5 && (
                    <>
                      ...
                      >
                        ...
                        {loading ? "Creating issue..." : "Open BMAD launchpad"}
                      </Button>
                    </>
                  )}
                </div>
