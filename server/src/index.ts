...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.deploymentMode === "local_trusted") {
    ...
    try {
      const bootstrap = await ensureOrkDefaultCompany(db as any, {
        actorUserId: LOCAL_BOARD_USER_ID,
        deploymentMode: config.deploymentMode,
        enabled: process.env.PAPERCLIP_ORK_AUTO_BOOTSTRAP !== "false",
        storage: storageService,
      });
      if (bootstrap.status === "created") {
        logger.info(
          {
            starterId: bootstrap.starterId,
            companyId: bootstrap.companyId,
            companyName: bootstrap.companyName,
            agentCount: bootstrap.agentCount,
            projectCount: bootstrap.projectCount,
            issueCount: bootstrap.issueCount,
            skillCount: bootstrap.skillCount,
          },
          "Bootstrapped the default ORK company into an empty local instance",
        );
      }
    } catch (err) {
      logger.error(
        { err },
        "Failed to bootstrap the default ORK company; startup will continue with manual onboarding fallback",
      );
    }
  }
