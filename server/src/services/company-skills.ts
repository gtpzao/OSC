...
function isBundledPaperclipImport(skill: Pick<ImportedSkill, "key" | "sourceType" | "metadata">) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  const sourceKind = asString(metadata?.sourceKind);
  if (sourceKind === "paperclip_bundled") return true;
...
export function companySkillService(db: Db) {
  ...
  async function ensureBundledSkills(companyId: string) {
    for (const skillsRoot of resolveBundledSkillsRoot()) {
      ...
      const bundledSkills = await readLocalSkillImports(companyId, skillsRoot)
        .then((skills) => skills
          // `paperclip-board` is used by the board chat runtime directly and
          // should not appear as a company skill inside imported orgs.
          .filter((skill) => skill.slug !== "paperclip-board")
          .map((skill) => ({
          ...skill,
          key: deriveCanonicalSkillKey(companyId, {
            ...skill,
  ...
  async function importPackageFiles(
    ...
    for (const importedSkill of importedSkills) {
      ...
      const slugConflictIsBundledPaperclip =
        existingByIncomingSlug
        && getSkillMeta(existingByIncomingSlug).sourceKind === "paperclip_bundled"
        && importedSkill.key !== existingByIncomingSlug.key;

      if (slugConflictIsBundledPaperclip) {
        out.push({
          skill: existingByIncomingSlug,
          action: "skipped",
          originalKey,
          originalSlug,
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (existing && existingMeta.sourceKind === "paperclip_bundled" && isBundledPaperclipImport(skill)) {
        out.push(existing);
        continue;
      }
