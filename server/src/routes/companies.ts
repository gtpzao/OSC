...
import { loadBundledCompanyStarter } from "../services/bundled-company-starters.js";

const bundledStarterInstallSchema = z.object({
  adapterType: z.string().min(1),
  adapterConfig: z.record(z.string(), z.unknown()),
  nameOverrides: z.record(z.string().min(1), z.string().min(1)).optional(),
  collisionStrategy: z.enum(["rename", "skip", "replace"]).optional(),
...
export function companyRoutes(db: Db, storage?: StorageService) {
  ...
  router.post(
    "/:companyId/starters/:starterId/install",
    validate(bundledStarterInstallSchema),
    async (req, res) => {
      const companyId = req.params.companyId as string;
      await assertCanManagePortability(req, companyId, "imports");
      const starter = await loadBundledCompanyStarter(req.params.starterId as string);
      const actor = getActorInfo(req);
      const adapterOverrides = Object.fromEntries(
        starter.agentSlugs.map((slug) => [
          slug,
          {
      ...
      const result = await portability.importBundle(
        {
          ...
          nameOverrides: req.body.nameOverrides,
      ...
      await logActivity(db, {
        ...
        agentId: actor.agentId,
        runId: actor.runId,
        action: "company.starter_installed",
        details: {
