...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  it("skips inline Paperclip clones when the bundled runtime skill already exists", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const bundledSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-bundled-clone-"));
    cleanupDirs.add(bundledSkillDir);
    await fs.writeFile(path.join(bundledSkillDir, "SKILL.md"), "---\nname: Paperclip\n---\n\n# Official Paperclip\n", "utf8");
    ...
    const [result] = await svc.importPackageFiles(companyId, {
      "skills/company/SOF/paperclip/SKILL.md": [
        "---",
        "name: Paperclip",
        "slug: paperclip",
        "---",
        "",
        "# Imported Paperclip Clone",
        "",
      ].join("\n"),
    }, { onConflict: "replace" });
    ...
    expect(result).toMatchObject({
      action: "skipped",
      originalKey: `company/${companyId}/paperclip`,
      originalSlug: "paperclip",
      reason: "Bundled Paperclip skill already exists; imported clone was ignored.",
      skill: {
        id: skillId,
        key: "paperclipai/paperclip/paperclip",
        sourceType: "local_path",
    ...
    const storedRows = (await db.select().from(companySkills))
      .filter((row) => row.companyId === companyId && row.slug === "paperclip");
    ...
    expect(storedRows[0]?.markdown).toContain("Official Paperclip");
    expect(storedRows[0]?.markdown).not.toContain("Imported Paperclip Clone");
  });

  it("keeps bundled Paperclip runtime sources when a package references the reserved skill", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const bundledSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-bundled-reference-"));
    cleanupDirs.add(bundledSkillDir);
    await fs.writeFile(path.join(bundledSkillDir, "SKILL.md"), "---\nname: Paperclip\n---\n\n# Official Paperclip\n", "utf8");
    ...
    expect(result).toMatchObject({
      action: "updated",
      originalKey: "paperclipai/paperclip/paperclip",
      originalSlug: "paperclip",
      skill: {
        id: skillId,
        key: "paperclipai/paperclip/paperclip",
    ...
    expect(stored?.markdown).toContain("Official Paperclip");
    expect(stored?.markdown).not.toContain("Referenced Paperclip");
  });
