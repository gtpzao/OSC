...
metadata:
  paperclip:
    slug: "paperclip-create-agent"
    skillKey: "paperclip-create-agent"
  paperclipSkillKey: "paperclip-create-agent"
  skillKey: "paperclip-create-agent"
...
- **Exact template** Ã¢â‚¬â€ the role matches an entry in the template index. Use the matching file under `references/agents/` as the starting point.
- **Adjacent template** Ã¢â‚¬â€ no exact match, but an existing template is close (for example, a "Backend Engineer" hire adapted from `coder.md`, or a "Content Designer" adapted from `uxdesigner.md`). Copy the closest template and adapt deliberately: rename the role, rewrite the role charter, swap domain lenses, and remove sections that do not fit.
- **Generic fallback** Ã¢â‚¬â€ no template is close. Use the baseline role guide to construct a new `AGENTS.md` from scratch, filling in each recommended section for the specific role.
