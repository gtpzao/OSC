...
metadata:
  ...
  paperclipSkillKey: "diagnose-why-work-stopped"
  skillKey: "diagnose-why-work-stopped"
...
- "liveness Ã¢â‚¬â€ what happened here", "this tree stopped working", "stuck"
...
### 1. Forensics on the named tree Ã¢â‚¬â€ before anything else
...
- Quote the evidence: run ids, comment timestamps, status transitions. "Inferred" is acceptable only when an API boundary blocks direct evidence Ã¢â‚¬â€ say so explicitly and mark the claim provisional ([PAP-2631](/PAP/issues/PAP-2631)).
...
State in the forensics: "I reviewed X, Y, Z. The new gap is Ã¢â‚¬Â¦"
...
- **Truly needs human or board intervention** Ã¢â‚¬â€ name the owner and the action.
- **Agent-actionable but not currently routed** Ã¢â‚¬â€ name the rule that would have routed it, and the agent that should have been waked.
- **Already covered** Ã¢â‚¬â€ point at the active run, queued wake, recovery issue, or pending interaction.
...
- Wait for board/CTO acceptance. If the user posts a new comment that supersedes the plan, the prior confirmation is invalidated Ã¢â‚¬â€ open a fresh confirmation tied to the new revision ([PAP-2602](/PAP/issues/PAP-2602) cycled three revisions; that is fine).
...
- **Hiding the chain.** Don't silently delete or hide the symptomatic recovery issues Ã¢â‚¬â€ the operator needs the audit trail.
