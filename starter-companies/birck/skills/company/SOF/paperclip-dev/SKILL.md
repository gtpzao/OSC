...
metadata:
  paperclip:
    slug: "paperclip-dev"
    skillKey: "paperclip-dev"
  paperclipSkillKey: "paperclip-dev"
  skillKey: "paperclip-dev"
key: "paperclip-dev"
required: false
---
...
Paperclip worktrees combine git worktrees with isolated Paperclip instances Ã¢â‚¬â€ each gets its own database, server port, and environment seeded from the primary instance.
...
## Forks Ã¢â‚¬â€ Prefer Pushing to a User Fork
...
Treat any remote whose URL points to `github.com:<user>/paperclip` (or `github.com/<user>/paperclip.git`) as the user's fork. Common names are `fork`, `<username>`, or `myfork`. The remote named `origin` or `upstream` that points at `paperclipai/paperclip` is the canonical upstream Ã¢â‚¬â€ do not push feature branches there if a fork exists.
...
If `git remote -v` shows only `paperclipai/paperclip` remotes (no user fork), fall back to pushing branches to `origin` as before. Do NOT create a fork on the user's behalf Ã¢â‚¬â€ ask first.
...
### Step 1 Ã¢â‚¬â€ Read the canonical files

You MUST read all three of these files before creating a PR:

1. **`.github/PULL_REQUEST_TEMPLATE.md`** Ã¢â‚¬â€ the required PR body structure
2. **`CONTRIBUTING.md`** Ã¢â‚¬â€ contribution conventions, PR requirements, and thinking-path examples
3. **`.github/workflows/pr.yml`** Ã¢â‚¬â€ CI checks that gate merge

### Step 2 Ã¢â‚¬â€ Validate your PR body against this checklist

After reading the template, verify your `--body` includes every one of these sections (names must match exactly):

- [ ] `## Thinking Path` Ã¢â‚¬â€ blockquote style, 5-8 reasoning steps
- [ ] `## What Changed` Ã¢â‚¬â€ bullet list of concrete changes
- [ ] `## Verification` Ã¢â‚¬â€ how a reviewer confirms this works
- [ ] `## Risks` Ã¢â‚¬â€ what could go wrong
- [ ] `## Model Used` Ã¢â‚¬â€ provider, model ID, version, capabilities
- [ ] `## Checklist` Ã¢â‚¬â€ copied from the template, items checked off

If any section is missing or empty, do NOT submit the PR. Go back and fill it in.

### Step 3 Ã¢â‚¬â€ Create the PR

Only after completing Steps 1 and 2, run `gh pr create`. Use the template contents as the structure for `--body` Ã¢â‚¬â€ do not write a freeform summary.

## Hard Rules Ã¢â‚¬â€ Do NOT Bypass
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   ```
   If any step fails, follow rule 2 Ã¢â‚¬â€ stop and report.
...
When an agent needs to start a dev server that outlives the current heartbeat Ã¢â‚¬â€ for example, so a human or QA agent can manually test against it Ã¢â‚¬â€ the server process **must** be launched in a detached session. A process started directly from a heartbeat shell is killed when the heartbeat exits.
...
2. **Name the session descriptively** Ã¢â‚¬â€ include the worktree name and port (e.g., `auth-fix-3102`).
3. **Verify the server is listening** before reporting the URL to anyone.
4. **Do not use `nohup` or `&` alone** Ã¢â‚¬â€ these are unreliable for agent shells that may have their entire process group killed.
5. **Clean up when done** Ã¢â‚¬â€ kill the tmux session when the testing is complete.
...
| CLI command fails | Do NOT work around it Ã¢â‚¬â€ report the error and block (see Hard Rules above) |
| Agent tries manual postgres operations | NEVER do this Ã¢â‚¬â€ all DB ops go through the CLI (see Hard Rules above) |
| Dev server dies between heartbeats | Launch in a detached `tmux` session Ã¢â‚¬â€ see "Persistent Dev Servers" above |
| Pushed feature branch to `paperclipai/paperclip` when a fork exists | Push to the user's fork remote instead Ã¢â‚¬â€ see "Forks" above |
