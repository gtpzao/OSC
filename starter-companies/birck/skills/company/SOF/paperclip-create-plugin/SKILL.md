...
metadata:
  paperclip:
    slug: "paperclip-create-plugin"
    skillKey: "paperclip-create-plugin"
  paperclipSkillKey: "paperclip-create-plugin"
  skillKey: "paperclip-create-plugin"
...
3. `doc/plugins/PLUGIN_SPEC.md` Ã¢â‚¬â€ future-looking context only
...
- `--output <dir>` Ã¢â‚¬â€ parent directory; the command creates `<dir>/<unscoped-name>/`. Defaults to the current directory.
- `--template <default|connector|workspace|environment>` Ã¢â‚¬â€ starter template.
- `--category <connector|workspace|automation|ui|environment>` Ã¢â‚¬â€ manifest category.
- `--display-name <name>`, `--description <text>`, `--author <name>` Ã¢â‚¬â€ manifest metadata.
- `--sdk-path <path>` Ã¢â‚¬â€ snapshot the local SDK from a Paperclip checkout into `.paperclip-sdk/` (useful when developing against an unreleased SDK).
...
- The server watches built outputs (`dist/`) for local-path plugins and restarts the plugin worker on rebuild Ã¢â‚¬â€ you do not need to reinstall after every edit.
...
- `src/manifest.ts` Ã¢â‚¬â€ declared capabilities and slots
- `src/worker.ts` Ã¢â‚¬â€ worker entry
- `src/ui/index.tsx` Ã¢â‚¬â€ UI entry (if applicable)
- `tests/plugin.spec.ts` Ã¢â‚¬â€ placeholder test
- `package.json` Ã¢â‚¬â€ `paperclipPlugin` block points at `dist/manifest.js`, `dist/worker.js`, `dist/ui/`
...
- **Scaffold path** Ã¢â‚¬â€ absolute path of the created plugin folder.
- **Commands run** Ã¢â‚¬â€ the exact `paperclipai plugin init`, `pnpm install`, `pnpm dev`, `paperclipai plugin install <path>` invocations (and any verification commands).
- **Install status** Ã¢â‚¬â€ output of `paperclipai plugin list` / `plugin inspect` (plugin key, version, status). Note if `status` is anything other than `ready` and include `lastError`.
- **Tests / build result** Ã¢â‚¬â€ `pnpm typecheck`, `pnpm test`, `pnpm build` pass/fail with the failing output if any.
- **Reload limitations** Ã¢â‚¬â€ call out anything that did not hot-reload (e.g. manifest changes required a reinstall, UI dev server was not wired, etc.).

If any item is missing, mark it as such Ã¢â‚¬â€ do not silently skip.
