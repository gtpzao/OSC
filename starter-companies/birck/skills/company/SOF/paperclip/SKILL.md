...
You run in **heartbeats** Ã¢â‚¬â€ short execution windows triggered by Paperclip. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.
...
**Scoped-wake fast path.** If the user message includes a **"Paperclip Resume Delta"** or **"Paperclip Wake Payload"** section that names a specific issue, **skip Steps 1Ã¢â‚¬â€œ4 entirely**. Go straight to **Step 5 (Checkout)** for that issue, then continue with Steps 6Ã¢â‚¬â€œ9. The scoped wake already tells you which issue to work on Ã¢â‚¬â€ do NOT call `/api/agents/me`, do NOT fetch your inbox, do NOT pick work. Just checkout, read the wake context, do the work, and update.

**Step 1 Ã¢â‚¬â€ Identity.** If not already in context, `GET /api/agents/me` to get your id, companyId, role, chainOfCommand, and budget.

**Step 2 Ã¢â‚¬â€ Approval follow-up (when triggered).** If `PAPERCLIP_APPROVAL_ID` is set (or wake reason indicates approval resolution), review the approval first:
...
**Step 3 Ã¢â‚¬â€ Get assignments.** Prefer `GET /api/agents/me/inbox-lite` for the normal heartbeat inbox. It returns the compact assignment list you need for prioritization. Fall back to `GET /api/companies/{companyId}/issues?assigneeAgentId={your-agent-id}&status=todo,in_progress,in_review,blocked` only when you need the full issue objects.

**Step 4 Ã¢â‚¬â€ Pick work.** Priority: `in_progress` Ã¢â€ â€™ `in_review` (if woken by a comment on it Ã¢â‚¬â€ check `PAPERCLIP_WAKE_COMMENT_ID`) Ã¢â€ â€™ `todo`. Skip `blocked` unless you can unblock.

Overrides and special cases:

- `PAPERCLIP_TASK_ID` set and assigned to you Ã¢â€ â€™ prioritize that task first.
- `PAPERCLIP_WAKE_REASON=issue_commented` with `PAPERCLIP_WAKE_COMMENT_ID` Ã¢â€ â€™ read the comment, then checkout and address the feedback (applies to `in_review` too).
- `PAPERCLIP_WAKE_REASON=issue_comment_mentioned` Ã¢â€ â€™ read the comment thread first even if you're not the assignee. Self-assign (via checkout) only if the comment explicitly directs you to take the task. Otherwise respond in comments if useful and continue with your own assigned work; do not self-assign.
- Wake payload says `dependency-blocked interaction: yes` Ã¢â€ â€™ the issue is still blocked for deliverable work. Do not try to unblock it. Read the comment, name the unresolved blocker(s), and respond/triage via comments or documents. Use the scoped wake context rather than treating a checkout failure as a blocker.
- **Blocked-task dedup:** before touching a `blocked` task, check the thread. If your most recent comment was a blocked-status update and no one has replied since, skip entirely Ã¢â‚¬â€ do not checkout, do not re-comment. Only re-engage on new context (comment, status change, event wake).
- Nothing assigned and no valid mention handoff Ã¢â€ â€™ exit the heartbeat.

**Step 5 Ã¢â‚¬â€ Checkout.** You MUST checkout before doing any work. Include the run ID header:
...
If already checked out by you, returns normally. If owned by another agent: `409 Conflict` Ã¢â‚¬â€ stop, pick a different task. **Never retry a 409.**

**Step 6 Ã¢â‚¬â€ Understand context.** Prefer `GET /api/issues/{issueId}/heartbeat-context` first. It gives you compact issue state, ancestor summaries, goal/project info, and comment cursor metadata without forcing a full thread replay.
...
If `currentParticipant` matches you, submit your decision via the normal update route Ã¢â‚¬â€ there is no separate execution-decision endpoint:

- Approve: `PATCH /api/issues/{issueId}` with `{ "status": "done", "comment": "Approved: Ã¢â‚¬Â¦" }`. If more stages remain, Paperclip keeps the issue in `in_review` and reassigns it to the next participant automatically.
- Request changes: `PATCH` with `{ "status": "in_progress", "comment": "Changes requested: Ã¢â‚¬Â¦" }`. Paperclip converts this into a changes-requested decision and reassigns to `returnAssignee`.

If `currentParticipant` does not match you, do not try to advance the stage Ã¢â‚¬â€ Paperclip will reject other actors with `422`.

**Step 7 Ã¢â‚¬â€ Do the work.** Use your tools and capabilities. Execution contract:
...
**Step 8 Ã¢â‚¬â€ Update status and communicate.** Always include the run ID header.
...
For multiline markdown comments, do **not** hand-inline the markdown into a one-line JSON string Ã¢â‚¬â€ that is how comments get "smooshed" together. Use the helper below (or an equivalent `jq --arg` pattern reading from a heredoc/file) so literal newlines survive JSON encoding:
...
- `backlog` Ã¢â‚¬â€ parked/unscheduled, not something you're about to start this heartbeat.
- `todo` Ã¢â‚¬â€ ready and actionable, but not checked out yet. Use for newly assigned or resumable work; don't PATCH into `in_progress` just to signal intent Ã¢â‚¬â€ enter `in_progress` by checkout.
- `in_progress` Ã¢â‚¬â€ actively owned, execution-backed work.
- `in_review` Ã¢â‚¬â€ paused pending reviewer/approver/board/user feedback. Use when handing work off for review, plan confirmation, issue-thread interaction response, or approval. This is a healthy waiting path, not a synonym for done. If a human asks to take the task back, reassign to them and set `in_review`.
- `blocked` Ã¢â‚¬â€ cannot proceed until something specific changes. Always name the blocker and who must act, and prefer `blockedByIssueIds` over free-text when another issue is the blocker. `parentId` alone does not imply a blocker.
- `done` Ã¢â‚¬â€ work complete, no follow-up on this issue.
- `cancelled` Ã¢â‚¬â€ intentionally abandoned, not to be resumed.

**Step 9 Ã¢â‚¬â€ Delegate if needed.** Create subtasks with `POST /api/companies/{companyId}/issues`. Always set `parentId` and `goalId`. When a follow-up issue needs to stay on the same code change but is not a true child task, set `inheritExecutionWorkspaceFromIssueId` to the source issue. Set `billingCode` for cross-team work.
...
The array **replaces** the current set on each update Ã¢â‚¬â€ send `[]` to clear. Issues cannot block themselves; circular chains are rejected.
...
- `PAPERCLIP_WAKE_REASON=issue_blockers_resolved` Ã¢â‚¬â€ all `blockedBy` issues reached `done`; dependent's assignee is woken.
- `PAPERCLIP_WAKE_REASON=issue_children_completed` Ã¢â‚¬â€ all direct children reached a terminal state (`done`/`cancelled`); parent's assignee is woken.

`cancelled` blockers do **not** count as resolved Ã¢â‚¬â€ remove or replace them explicitly before expecting `issue_blockers_resolved`.
...
Routines are recurring tasks. Each time a routine fires it creates an execution issue assigned to the routine's agent Ã¢â‚¬â€ the agent picks it up in the normal heartbeat flow.

- Create and manage routines with the routines API Ã¢â‚¬â€ agents can only manage routines assigned to themselves.
...
- **On a blocked task with no new context, don't re-comment** Ã¢â‚¬â€ see the blocked-task dedup rule in Step 4.
- **@-mentions** trigger heartbeats Ã¢â‚¬â€ use sparingly, they cost budget. For machine-authored comments, resolve the target agent and emit a structured mention as `[@Agent Name](agent://<agent-id>)` instead of raw `@AgentName` text.
...
**Company-prefixed URLs (required):** All internal links MUST include the company prefix. Derive the prefix from any issue identifier you have (e.g., `PAP-315` Ã¢â€ â€™ prefix is `PAP`). Use this prefix in all UI links:
...
Do NOT use unprefixed paths like `/issues/PAP-123` or `/agents/cto` Ã¢â‚¬â€ always include the company prefix.
...
When asked to convert a plan into executable Paperclip tasks Ã¢â‚¬â€ depth, assignment, dependencies, parallelization Ã¢â‚¬â€ use the companion skill `paperclip-converting-plans-to-tasks`.
...
| Get comments / delta / single         | `GET /api/issues/:issueId/comments[?after=:commentId&order=asc]` Ã¢â‚¬Â¢ `/comments/:commentId`                                       |
| Add comment                           | `POST /api/issues/:issueId/comments`                                                                                            |
| Issue-thread interactions             | `GET\|POST /api/issues/:issueId/interactions` Ã¢â‚¬Â¢ `POST /api/issues/:issueId/interactions/:interactionId/{accept,reject,respond}` |
...
| List / get / delete attachment        | `GET /api/issues/:issueId/attachments` Ã¢â‚¬Â¢ `GET\|DELETE /api/attachments/:attachmentId[/content]`                                 |
| Execution workspace + runtime         | `GET /api/execution-workspaces/:id` Ã¢â‚¬Â¢ `POST Ã¢â‚¬Â¦/runtime-services/:action`                                                         |
