...
export async function resolveWindowsUserLocalBinCommandPath(
  command: string,
  env: NodeJS.ProcessEnv,
  ...
  const homeDir = env.USERPROFILE?.trim() || env.HOME?.trim() || os.homedir().trim();
  if (!homeDir) return null;

  const localBinDir = path.resolve(homeDir, ".local", "bin");
  const hasExtension = path.extname(command).length > 0;
  const candidates = hasExtension
    ? [path.join(localBinDir, command)]
    : windowsPathExts(env).map((ext) => path.join(localBinDir, `${command}${ext}`));
...
async function resolveCommandPath(command: string, cwd: string, env: NodeJS.ProcessEnv): Promise<string | null> {
  ...
  // Free Claude Code's Windows installer writes launchers into the user's
  // ~/.local/bin directory and updates the user PATH. Long-lived Paperclip
  // dev servers can miss that PATH mutation until they restart, so probe the
  // standard install location before declaring the command missing.
  const windowsUserLocalBinResolved = await resolveWindowsUserLocalBinCommandPath(command, env);
  if (windowsUserLocalBinResolved) return windowsUserLocalBinResolved;
