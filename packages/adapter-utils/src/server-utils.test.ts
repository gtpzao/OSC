...
import {
  ...
  resolveWindowsUserLocalBinCommandPath,
  runningProcesses,
  runChildProcess,
...
describe("resolveWindowsUserLocalBinCommandPath", () => {
  it("resolves extensionless commands from USERPROFILE/.local/bin on Windows", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-user-local-bin-"));
    try {
      const localBinDir = path.join(root, ".local", "bin");
      const launcher = path.join(localBinDir, "fcc-claude.exe");
      await fs.mkdir(localBinDir, { recursive: true });
      await fs.writeFile(launcher, "", "utf8");
      ...
      ).resolves.toSatisfy((resolved) => resolved?.toLowerCase() === launcher.toLowerCase());
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });

  it("skips the fallback outside Windows", async () => {
    await expect(
      resolveWindowsUserLocalBinCommandPath(
        "fcc-claude",
        {
