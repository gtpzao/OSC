...
export function resolveClaudeHelloProbeCommand(command: string): "claude" | "fcc-claude" | null {
  if (commandLooksLike(command, "claude")) return "claude";
  if (commandLooksLike(command, "fcc-claude")) return "fcc-claude";
  return null;
}

export function buildClaudeCommandUnresolvableHint(command: string): string | null {
  if (resolveClaudeHelloProbeCommand(command) !== "fcc-claude") return null;
  return "Install the Free Claude Code launcher so `fcc-claude` is on PATH, or point Command at the absolute path to the installed `fcc-claude` executable.";
}

function buildClaudeManualProbeHint(command: string): string {
  return `Run \`${command} --print - --output-format stream-json --verbose\` manually in this directory and prompt \`Respond with hello\` to debug.`;
}
...
export async function testEnvironment(
  ...
  try {
    ...
    checks.push({
      ...
      detail: command,
      ...(buildClaudeCommandUnresolvableHint(command)
        ? { hint: buildClaudeCommandUnresolvableHint(command) }
        : {}),
    });
  }
  ...
  if (canRunProbe) {
    const helloProbeCommand = resolveClaudeHelloProbeCommand(command);
    if (!helloProbeCommand) {
      checks.push({
        code: "claude_hello_probe_skipped_custom_command",
        level: "info",
        message: "Skipped hello probe because command is not a supported Claude launcher.",
        detail: command,
        hint: "Use `claude` or `fcc-claude` to run the automatic login and installation probe.",
      });
    } else {
      ...
      if (probe.timedOut) {
        checks.push({
          ...
          hint: `Retry the probe. If this persists, verify ${command} can run \`Respond with hello\` from this directory manually.`,
        });
        ...
        checks.push({
          ...
          ...(hasHello
            ? {}
            : {
                hint: buildClaudeManualProbeHint(command),
              }),
        });
      } else {
        checks.push({
