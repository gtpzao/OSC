...
const EMBEDDED_POSTGRES_CLEANUP_RETRYABLE_CODES = new Set(["EBUSY", "ENOTEMPTY", "EPERM"]);
const EMBEDDED_POSTGRES_CLEANUP_RETRIES = 10;
const EMBEDDED_POSTGRES_CLEANUP_RETRY_DELAY_MS = 100;
...
function isRetryableCleanupError(error: unknown): boolean {
  ...
  return typeof code === "string" && EMBEDDED_POSTGRES_CLEANUP_RETRYABLE_CODES.has(code);
}

async function cleanupEmbeddedPostgresTestDirs(dataDir: string) {
  for (let attempt = 0; attempt <= EMBEDDED_POSTGRES_CLEANUP_RETRIES; attempt += 1) {
    try {
      ...
      if (
        ...
        attempt === EMBEDDED_POSTGRES_CLEANUP_RETRIES
      ) {
        throw error;
      }
      await new Promise((resolve) =>
        setTimeout(resolve, EMBEDDED_POSTGRES_CLEANUP_RETRY_DELAY_MS * (attempt + 1)));
    }
  }
}

async function cleanupEmbeddedPostgresTestDirsBestEffort(dataDir: string) {
  await cleanupEmbeddedPostgresTestDirs(dataDir).catch(() => {});
}
...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  ...
  try {
    ...
    if (dataDir) await cleanupEmbeddedPostgresTestDirsBestEffort(dataDir);
  }
}
